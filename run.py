...
def run(input_path, output_path, model_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
		...
		model, transform, net_w, net_h = load_model(device, model_path, model_type, optimize, height, square)
		...
		#if (model_type == "dpt_swin2_large_384"):
				#model.load_state_dict(torch.load("./pruned_swinl.pt"))
				#backend ='qnnpack'
				#model.float16()
				#model.train()
				...
				#print(model.pretrained.get_device())
				#print(model_fp32_prepared.get_device())


				#model_bfloat16 = torch.quantization.convert(model_fp32_prepared, inplace=False).to(device)
				#model_bfloat16 = torch.quantization.convert(model, mapping={"float32": "bfloat16"})

				#print([p[1].dtype for p in model_bfloat16.named_parameters()])
				#print(model_bfloat16)
				#exit()

				#model_bfloat16.load_state_dict(torch.load("./weights/dpt_swin2_large_384_qunt.pt"))
				#model = model_bfloat16
				#print("Loaded pruned model: ({} parameters)".format( \
		...
		if input_path is not None:
				...
				with torch.no_grad():
						fps = 1
						video = FileVideoStream("video/waucissa.mov").start()
						...
						while True:
								...
								if frame is not None:
										...
										if cv2.waitKey(1) == ord('q'):	# Escape key
												break
...
if __name__ == "__main__":
		...
		parser.add_argument('--fps',
												action='store_true',
												help='If specified runs FPS test of movie file'
												)
